    ...
    public function wpgs_enqueue_files()
    {
        ...
        if ($zoom == 'true' && $mobile_zoom == 'true') {
            ...
            $wpgs_setting_css .= "
				...
				";
        } elseif ($zoom == 'true' && $mobile_zoom == 'false' && !wp_is_mobile()) {
            $zoom_js = "
			$('.wpgs-image img').each(function () {
				$(this).wrap('<div class=\'zoom_w\' style=\'display: inline-block; position: relative; overflow: hidden;\'></div>');
				var newImgSrc = $(this).attr('data-large_image');
        ...
        if ($mobile_zoom == 'false' && wp_is_mobile()) {
			...
			";
        } elseif ($zoom == 'true' && cix_wpgs::option('lightbox_icon') == 'none' && !wp_is_mobile()) {
            $wpgs_setting_css .="
			a.woocommerce-product-gallery__lightbox {
...
if (! function_exists('wpgs_get_image_gallery_html')) {
    ...
    function wpgs_get_image_gallery_html($attachment_id, $main_image = false, $size = 'woocommerce_single')
    {
        ...
        if ($size == 'woocommerce_single' && $lightbox == 'true') {
            ...
            return $markup;
        } elseif ($size == apply_filters('wpgs_new_thumb_img_size', 'woocommerce_gallery_thumbnail') && apply_filters('wpgs_carousel_mode', true) == false) {
