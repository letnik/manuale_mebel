...
class Product
{
    ...
    public function add_cart_item_data($cart_item_meta, $product_id, $variation_id, $post_data = null)
    {
        ...
        $groups = $variation_id ? Group::getByProduct($variation_id, false, true) : Group::getByProduct($product_id);
    ...
    public function cart_adjust_price($cart_item)
    {
        ...
        if (!empty($zaddon)) {
            ...
            $price = get_option( 'woocommerce_tax_display_shop' ) === 'incl' ? wc_get_price_including_tax( $product ) : wc_get_price_excluding_tax( $product );
            $additional = array_reduce($groups, function ($total, $group) use ($zaddon, $price) {
                $groupAddon = sizeof($group['types']) > 0 && isset($zaddon[$group['id']])? $zaddon[$group['id']] : [];
                return $groupAddon
                    ? array_reduce($group['types'], function ($total, $type) use ($groupAddon, $price) {
                        $typeAddon = isset($groupAddon[$type['id']]) ? $groupAddon[$type['id']] : [];
                        return array_reduce($type['values'], function ($total, $value) use ($typeAddon, $type, $price) {
                            ...
                            $addon_value = $selected ? $quantity * $value['price'] : 0;
    ...
    protected static function item_meta($product_id, $zaddon, $variation = false)
    {
        ...
        $zaddon_meta = array_reduce($groups, function ($groups, $group) use ($zaddon, $price) {
            $groupAddon = sizeof($group['types']) > 0 && isset($zaddon[$group['id']])? $zaddon[$group['id']] : [];
            $types = $groupAddon ? array_reduce($group['types'], function ($types, $type) use ($groupAddon, $price) {
                $typeAddon = isset($groupAddon[$type['id']]) ? $groupAddon[$type['id']] : [];
								$valuesMeta = array_reduce($type['values'], function ($acc, $value) use ($typeAddon, $type, $price) {
								    $usedAddon = false;
                                    switch ($type['type']) {
                                        case 'select':
                                        case 'radio':
    ...
    public function enqueue_scripts()
    {
        if (is_single() && is_product()) {
            wp_enqueue_script('za_product.js', plugins_url('assets/product.js', \ZAddons\PLUGIN_ROOT_FILE), ['jquery'], '1.3');
